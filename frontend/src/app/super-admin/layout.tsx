...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ProtectedRoute allowedRole={RoleName.SUPER_ADMIN}>
      <div className="min-h-screen bg-background text-foreground flex overflow-hidden transition-colors duration-500">
        <SuperAdminSidebar isOpen={isSidebarOpen} onLogout={logout} />

        <div className="flex-1 flex flex-col relative overflow-hidden">
          <SuperAdminHeader user={user} onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
