...
export const societiesService = {
  ...
  async onboardSociety(data: OnboardSocietyInput): Promise<OnboardSocietyResponse> {
    try {
      const response = await apiClient.post<ApiResponse<OnboardSocietyResponse>>(
        '/societies',
        data
      );
      return response.data.data;
  ...
  async addAdmin(societyId: string, data: Record<string, unknown>): Promise<AdminUser> {
    try {
      const response = await apiClient.post<ApiResponse<AdminUser>>(
        `/admin/super/societies/${societyId}/admins`,
        data
      );
      return response.data.data;
    } catch (error) {
  ...
  async getMySociety(): Promise<Society> {
    try {
      const response = await apiClient.get<ApiResponse<Society>>('/societies/profile/me');
      return response.data.data;
    } catch (error) {
      errorService.handleError(error, 'error');
      throw error;
  ...
  async updateMySociety(data: Partial<Society>): Promise<Society> {
    try {
      const response = await apiClient.patch<ApiResponse<Society>>('/societies/profile/me', data);
      return response.data.data;
    } catch (error) {
      errorService.handleError(error, 'error');
      throw error;
