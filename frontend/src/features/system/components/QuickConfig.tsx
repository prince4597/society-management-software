    ...
    <Card variant="flat" padding="none">
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-secondary/10 border border-border/50 rounded-lg">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <ShieldAlert size={16} className={maintenance?.value ? "text-rose-500" : "text-muted-foreground/60"} />
              <span className="text-sm font-bold text-foreground uppercase tracking-tight">Maintenance Mode</span>
              <Badge variant={maintenance?.value ? 'error' : 'success'} size="sm">
                {maintenance?.value ? "Active" : "Disabled"}
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground font-medium">Instantly disable non-admin operations across all system nodes.</p>
          </div>
          <Button
            variant="outline"
            size="sm"
            ...
            className={cn(
              ...
              maintenance?.value ? "hover:bg-rose-500/10 hover:text-rose-500" : "hover:bg-emerald-500/10 hover:text-emerald-500"
            )}
          >
            {updating === 'maintenance_mode' ? "Processing..." : (maintenance?.value ? "Disable" : "Enable")}
          </Button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-secondary/10 border border-border/50 rounded-lg">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Globe size={16} className={registration?.value ? "text-primary/70" : "text-muted-foreground/60"} />
              <span className="text-sm font-bold text-foreground uppercase tracking-tight">Public Onboarding</span>
              <Badge variant={registration?.value ? 'success' : 'warning'} size="sm">
                {registration?.value ? "Permitted" : "Paused"}
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground font-medium">Control if new societies can onboard via the public portal.</p>
          </div>
          <Button
            variant="outline"
            size="sm"
