...
import { AdminUser, RoleName } from '@/types';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = useCallback(async () => {
    ...
    try {
      ...
      localStorage.removeItem('sm_session_active');
      handleUnauthenticated();
    }
  }, [handleUnauthenticated]);
  ...
  useEffect(() => {
    if (status === 'idle') {
      const hasSessionHint = localStorage.getItem('sm_session_active') === 'true';
  ...
  const login = useCallback(
    (userData: AdminUser) => {
      ...
      setStatus('authenticated');
      setError(null);
      const redirectPath = userData.role === RoleName.SUPER_ADMIN ? '/super-admin' : '/dashboard';
  ...
  return (
    <AuthContext.Provider value={value}>
      {status === 'loading' ? <PremiumSplash /> : children}
